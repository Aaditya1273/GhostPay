...
/**
 * Persist a reasoning step to Walrus.
 *
 * The reasoning JSON includes:
 *   - The full reasoning record (observations, options, justification)
 *   - The context snapshot (balances, payments, preferences)
 *   - The resulting plan
 *
 * Returns the blob ID.
 */
export async function persistReasoningToWalrus(
  plan: AgentPlan,
  encrypt: boolean = true,
): Promise<PersistReasoningResult> {
  // Build the full reasoning payload
  ...
  // Upload to Walrus (epochs=30 for long-term retention)
  const result = await uploadToWalrus(bytes, 30, false);

  // Note: SEAL encryption would happen here in production.
  // For now we store as public (non-encrypted) blobs since the
  // SEAL key server dependency is not yet configured for agent use.
  // The payload does not contain private keys or secrets.
 ...
 * Returns the label and data type for the storeMemory call.
 */
...
/**
 * Execute a single agent action through the provided executor function.
 * Returns the result or throws.
 *
 * @param action  The action to execute
 * @param executor  Function that takes a Transaction and executes it
 */
export async function executeAgentAction(
  action: AgentAction,
  _executor: (tx: Transaction) => Promise<string>,
): Promise<string> {
  switch (action.type) {
    case "noop":
      return "noop";
    ...
    case "record_payment":
    case "store_memory":
      // These are executed by the React hook layer which has access to
      // the specific transaction hooks (useDeepBook, usePaymentTransaction, etc.)
      // The engine layer just describes WHAT to do; the hook layer does HOW.
      throw new Error(
        `Action ${action.type} must be executed by the hook layer, not the engine.`,
      );
  }
}
